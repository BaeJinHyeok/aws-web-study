...
//@NoArgsConstructor// SessionUser 에서 user.getName() 에 대한 NullException error 생성으로 인해 무한 리다이렉트-> 접속안됨 해결. 하지만 google 로그인이 안됨..? userName이 없는 건 해결안됨..
@Getter
public class SessionUser implements Serializable { // SessionUser 에는 인증된 사용자 정보만 필요함 그 외의 정보필요는 없으니 name,email.picture 만 필드로 선언
    ...
    private static final long serialVersionUID = 1L; // 직렬화 버전 UID. 없으면 Failed to convert from type [byte[]] to type 오류발생
